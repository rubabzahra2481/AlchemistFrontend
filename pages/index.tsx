...
export default function Home() {
  ...
  const [decisionIntelligenceMode, setDecisionIntelligenceMode] = useState(false);

  const handleSendMessage = async (message: string, selectedLLM?: string, onStream?: (chunk: any) => void) => {
    try {
      // Use deployed backend URL for production; local backend when on localhost or 127.0.0.1
      ...
      if (useStreaming) {
        ...
        const response = await fetch(apiUrl, {
          ...
          body: JSON.stringify({
            ...
            decisionIntelligenceMode: decisionIntelligenceMode,
          }),
        });
  ...
  return (
    <ChatInterface 
      ...
      decisionIntelligenceMode={decisionIntelligenceMode}
      onToggleDecisionMode={() => setDecisionIntelligenceMode(prev => !prev)}
    />
  );
}
